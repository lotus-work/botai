...
@Component({
  ...
  styleUrls: ['./loadchat.component.css']
})
export class LoadchatComponent {
  ...
  ngOnInit() {
    ...
    if (this.selectedConversationId && this.selectedUserId) {
      ...
      this.loadConversation(this.selectedConversationId, this.selectedUserId);
    }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if(this.selectedUserId !="" && this.selectedConversationId !=""){

      this.loadConversation(this.selectedConversationId, this.selectedUserId);
      this.conversationId = this.selectedConversationId;
    }
  ...
  loadConversation(convoId: string, userId: string): void {
    this._apiCallServices.getConversation(convoId, userId)
      .pipe(
        catchError(error => {
      ...
      .subscribe(response => {
        if (response?.isSuccessful && response.result?.messages) {
          // Initialize the `chatMessages` array with separate entries for each `botReply` and `userPrompt`
          this.chatMessages = [];
  
          response.result.messages.forEach((message: any) => {
            // Check if there's a user prompt and add it to chatMessages
            if (message.userPrompt) {
              this.chatMessages.push({
                role: 'user',
                content: this.sanitizer.bypassSecurityTrustHtml(message.userPrompt)
              });
            }
  
            // Check if there's a bot reply and add it to chatMessages
            if (message.botReply) {
              this.chatMessages.push({
                role: 'bot',
                content: this.sanitizer.bypassSecurityTrustHtml(message.botReply)
              });
            }
          });
  
          // Log the structured messages for verification
          console.log(this.chatMessages);
        }
      });
  }

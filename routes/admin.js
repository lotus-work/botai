    ...
    try {
        if ((!emailAddress && !username) || !password) {
            return sendResponse(res, 400, false, {
                message: "Username or email address and password are required.",
            });
        }
...
router.get("/users", async (req, res) => {
    try {
        ...
        for (let user of users) {
            const gptAssistant =
                (await GptAssistants.findOne({ userId: user._id })) || {}; // Default to empty object
            const organization =
                (await Organizations.findOne({ userId: user._id })) || {}; // Default to empty object
...
router.post("/user/add", async (req, res) => {
    ...
    try {
        ...
        const settings = await Settings.findOne({});
        const appNameFromSettings = settings?.appName || 'CC AI';
        ...
        if (assistantName && assistantId) {
            ...
            const text = `Hello ${name}, you have been onboarded. 
    ${appNameFromSettings} welcomes you!
    ...
    Please login here: https://chatgptbotai.netlify.app/`;
            ...
            const html = `
                ...
                <p><strong>${appNameFromSettings}</strong> welcomes you!</p>
...
router.put("/user/edit/gptassistant/:userId", async (req, res) => {
    ...
    try {
        ...
        if (!updatedAssistant) {
            return ApiResponse.sendResponse(res, 404, false, {
                message: "Assistant not found for this user.",
            });
        }
        ...
        ApiResponse.sendResponse(res, 500, false, {
            message: "Error updating assistant: " + err.message,
        });
    }
});
...
router.put("/user/edit/:userId", async (req, res) => {
    ...
    try {
        ...
        // Update user details
        const updatedUser = await Users.findByIdAndUpdate(userId, updatePayload, {
            new: true,
        });
...
router.get("/settings", async (req, res) => {
    try {
        ...
        sendResponse(res, 500, false, {
            message: "Error fetching settings: " + err.message,
        });
    }
});
    ...
    try {
        ...
        updateEnvFile({
            ...
            SYSTEM_EMAIL_USERNAME:
                systemEmail.username || settings.systemEmail.username,
            SYSTEM_EMAIL_ADDRESS:
                systemEmail.emailAddress || settings.systemEmail.emailAddress,
            SYSTEM_EMAIL_PASSWORD:
                systemEmail.password || settings.systemEmail.password,
            ...
            CHATGPT_MASTER_INSTRUCTION:
                chatGPT.masterInstruction || settings.chatGPT.masterInstruction,
            COMPANY_PHONE: companyContact.phone || settings.companyContact.phone,
            COMPANY_EMAIL:
                companyContact.emailAddress || settings.companyContact.emailAddress,
            COMPANY_ADDRESS:
                companyContact.fullAddress || settings.companyContact.fullAddress,
            WEBSITE_MAIN_LOGO_PATH:
                websiteContent.mainLogoPath || settings.websiteContent.mainLogoPath,
            WEBSITE_FAVICON_PATH:
                websiteContent.faviconPath || settings.websiteContent.faviconPath,
...
router.delete("/user/:userId", async (req, res) => {
    ...
    try {
        ...
        const organization = await Organizations.findOne({ userId }).session(
            session
        );
        if (organization) {
            ...
            const organizationMembers = await OrganizationMembers.find({
                organizationId,
            }).session(session);
            ...
            await Conversation.deleteMany({ userId: { $in: memberUserIds } }).session(
                session
            );
            await Message.deleteMany({ userId: { $in: memberUserIds } }).session(
                session
            );
        ...
        res.status(500).json({
            ...
            isSuccessful: false,
            result: {
                message: "Error deleting user and associated records: " + err.message,
            },
        });
    }
});
...
router.delete(
    ...
    async (req, res) => {
        ...
        try {
            ...
            const memberRecord = await OrganizationMembers.findOne({
                organizationId,
                userId,
            });
            if (!memberRecord) {
            ...
            const memberDelete = await OrganizationMembers.deleteOne(
                { organizationId, userId },
                { session }
            );
            const userDelete = await Users.deleteOne({ _id: userId }, { session });
            const conversationsDelete = await Conversation.deleteMany(
                { userId: userId },
                { session }
            );
...
router.get("/user/message-stats", async (req, res) => {
    ...
    try {
        ...
        if (organization) {
            const members = await OrganizationMembers.find({
        ...
        const userIdsAsObjectId = userIds.map(
...
router.get("/organization/message-stats/:organizationId", async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(organizationId)) {
        return ApiResponse.sendResponse(res, 400, false, {
    ...
    try {
        ...
        if (ownerUserId) {
            ...
            const owner =
                (await OrganizationMembers.findOne({ userId: ownerUserId })) ||
        ...
        if (members.length === 0) {
            return ApiResponse.sendResponse(res, 200, false, {
...
router.get("/basicDashboardStatistics", async (req, res) => {
    try {
        ...
        const totalNonOwners = await Users.countDocuments({
            role: { $ne: "User" },
        });
        const settings = await Settings.findOne({});
        const currentGptModel =settings?.currentGptModel || "not set";
